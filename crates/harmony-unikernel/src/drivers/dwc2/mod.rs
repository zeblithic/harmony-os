...
use fifo::{dieptxf_value, gnptxfsiz_value, RX_FIFO_WORDS, TX1_FIFO_WORDS, TX1_START, TX3_FIFO_WORDS, TX3_START};
use regs::*;
...
impl Dwc2Controller {
    ...
    pub fn init(bank: &mut impl RegisterBank) -> Result<(Self, Vec<Dwc2Action>), Dwc2Error> {
        ...
        actions.push(Dwc2Action::WriteRegister { offset: GUSBCFG, value: gusbcfg });

        // Configure FIFO sizes
        bank.write(GRXFSIZ, RX_FIFO_WORDS);
        actions.push(Dwc2Action::WriteRegister { offset: GRXFSIZ, value: RX_FIFO_WORDS });

        let gnptxfsiz = gnptxfsiz_value();
        bank.write(GNPTXFSIZ, gnptxfsiz);
        actions.push(Dwc2Action::WriteRegister { offset: GNPTXFSIZ, value: gnptxfsiz });

        // EP1 TX FIFO
        let dieptxf1 = dieptxf_value(TX1_START, TX1_FIFO_WORDS);
        bank.write(dieptxf(1), dieptxf1);
        actions.push(Dwc2Action::WriteRegister { offset: dieptxf(1), value: dieptxf1 });

        // EP3 TX FIFO
        let dieptxf3 = dieptxf_value(TX3_START, TX3_FIFO_WORDS);
        bank.write(dieptxf(3), dieptxf3);
        actions.push(Dwc2Action::WriteRegister { offset: dieptxf(3), value: dieptxf3 });

        // Set HS speed in DCFG
        bank.write(DCFG, DCFG_DEVSPD_HS);
        actions.push(Dwc2Action::WriteRegister { offset: DCFG, value: DCFG_DEVSPD_HS });

        // Enable global interrupts (GAHBCFG)
        bank.write(GAHBCFG, GAHBCFG_GLBL_INTR_EN);
        actions.push(Dwc2Action::WriteRegister { offset: GAHBCFG, value: GAHBCFG_GLBL_INTR_EN });
        ...
        actions.push(Dwc2Action::WriteRegister { offset: GINTMSK, value: gintmsk });

        // Unmask EP0 IN and EP0 OUT in DAINTMSK
        let daintmsk: u32 = (1 << 0) | (1 << 16); // EP0 IN bit 0, EP0 OUT bit 16
        bank.write(DAINTMSK, daintmsk);
        actions.push(Dwc2Action::WriteRegister { offset: DAINTMSK, value: daintmsk });

        // Enable transfer-complete interrupts for IN EPs (DIEPMSK)
        bank.write(DIEPMSK, DEPINT_XFERCOMPL);
        actions.push(Dwc2Action::WriteRegister { offset: DIEPMSK, value: DEPINT_XFERCOMPL });
        ...
        actions.push(Dwc2Action::WriteRegister { offset: DOEPMSK, value: doepmsk });
        ...
        actions.push(Dwc2Action::WriteRegister { offset: doeptsiz(0), value: doeptsiz0 });

        let doepctl0 = EPCTL_EPENA | EPCTL_CNAK;
        bank.write(doepctl(0), doepctl0);
        actions.push(Dwc2Action::WriteRegister { offset: doepctl(0), value: doepctl0 });

        // Soft-connect: clear SFTDISCON bit in DCTL
        let dctl = bank.read(DCTL) & !DCTL_SFTDISCON;
        bank.write(DCTL, dctl);
        actions.push(Dwc2Action::WriteRegister { offset: DCTL, value: dctl });
    ...
    pub fn handle_event(
        ...
        match event {
            Dwc2Event::BusReset => Ok(Self::handle_bus_reset(bank, &mut self.state, &mut self.address)),
            Dwc2Event::EnumerationDone { speed } => {
                self.speed = speed;
                Ok(Vec::new())
            }
            Dwc2Event::SetupReceived { data } => {
                self.handle_setup(data, bank)
            }
            Dwc2Event::RxFifoNonEmpty => {
                self.handle_rx_fifo(bank)
            }
    ...
    fn handle_rx_fifo(
        ...
        match pktsts {
            ...
            PKTSTS_OUT_COMPLETE if ep > 0 => {
                ...
                bank.write(doepctl(ep), bank.read(doepctl(ep)) | EPCTL_EPENA | EPCTL_CNAK);
                Ok(Vec::new())
            }
            _ => {
    ...
    pub fn submit_request(
        ...
        match req {
            ...
            GadgetRequest::ControlAck => {
                ...
                Self::write_ep0_in(bank, &[]);
                Ok(alloc::vec![Dwc2Action::WriteTxFifo { ep: 0, data: alloc::vec![] }])
            }
...
mod tests {
    ...
    fn init_forces_device_mode() {
        ...
        assert_ne!(val & GUSBCFG_FORCE_DEV, 0, "GUSBCFG_FORCE_DEV bit must be set");
    }
    ...
    fn init_configures_fifos() {
        ...
        assert_eq!(*val, gnptxfsiz_value(), "GNPTXFSIZ should match gnptxfsiz_value()");
    }
    ...
    fn init_soft_connects() {
        ...
        assert!(dctl_write.is_some(), "DCTL should be written for soft-connect");
        let (_, val) = dctl_write.unwrap();
        assert_eq!(val & DCTL_SFTDISCON, 0, "SFTDISCON bit must be cleared in DCTL");
    }
    ...
    fn bus_reset_returns_to_default() {
        ...
        assert_eq!(ctrl.state(), UsbDeviceState::Default, "should return to Default after reset");
        assert_eq!(ctrl.address, 0, "address should be cleared after reset");
        assert!(events.contains(&GadgetEvent::Reset), "should emit GadgetEvent::Reset");
    }
    ...
    fn enumeration_done_sets_speed() {
        ...
        ctrl.handle_event(Dwc2Event::EnumerationDone { speed: DeviceSpeed::FullSpeed }, &mut bank)
            .expect("handle_event failed");

        assert_eq!(ctrl.speed(), DeviceSpeed::FullSpeed, "speed should be FullSpeed");
    }
    ...
    fn set_address_programs_dcfg() {
        ...
        let dcfg_write = bank.writes.iter().find(|(off, val)| {
            *off == DCFG && (val & DCFG_DAD_MASK) == dcfg_dad(5)
        });
        assert!(dcfg_write.is_some(), "DCFG should be written with address 5");
        assert_eq!(ctrl.state(), UsbDeviceState::Address, "state should be Address");
    }
    ...
    fn set_configuration_enables_endpoints() {
        ...
        assert_eq!(ctrl.state(), UsbDeviceState::Configured, "state should be Configured");

        // Verify EP1 IN was activated
        let ep1_write = bank.writes.iter().find(|(off, val)| {
            *off == diepctl(1) && (val & EPCTL_USBAEP) != 0
        });
        assert!(ep1_write.is_some(), "EP1 IN should be activated");

        // Verify EP2 OUT was activated
        let ep2_write = bank.writes.iter().find(|(off, val)| {
            *off == doepctl(2) && (val & EPCTL_USBAEP) != 0
        });
        assert!(ep2_write.is_some(), "EP2 OUT should be activated");

        // Verify EP3 IN was activated
        let ep3_write = bank.writes.iter().find(|(off, val)| {
            *off == diepctl(3) && (val & EPCTL_USBAEP) != 0
        });
    ...
    fn class_request_forwarded_to_gadget() {
        ...
        assert!(
            events.iter().any(|e| matches!(e, GadgetEvent::SetupClassRequest { .. })),
    ...
    fn unsupported_standard_request_stalls() {
        ...
        let ep0_in_stall = bank.writes.iter().any(|(off, val)| {
            *off == diepctl(0) && (val & EPCTL_STALL) != 0
        });
        assert!(ep0_in_stall, "EP0 IN should be stalled for unknown standard request");
    }
    ...
    fn rx_fifo_bulk_out_produces_gadget_event() {
        ...
        let bulk_out = events.iter().find(|e| matches!(e, GadgetEvent::BulkOut { ep: 2, .. }));
        assert!(bulk_out.is_some(), "should produce GadgetEvent::BulkOut for ep=2");

        if let Some(GadgetEvent::BulkOut { data, .. }) = bulk_out {
            assert_eq!(data.len(), 4, "bulk out data should be exactly bcnt=4 bytes");
            assert_eq!(&data[..], &0xDEADBEEFu32.to_le_bytes(), "data should match FIFO words");
        }
    }
    ...
    fn submit_bulk_in_before_configured_fails() {
        ...
        let result = ctrl.submit_request(
            GadgetRequest::BulkIn { ep: 1, data: alloc::vec![0u8; 64] },
            &mut bank,
        );
    ...
    fn submit_bulk_in_writes_tx_fifo() {
        ...
        let fifo_action = actions.iter().find(|a| matches!(a, Dwc2Action::WriteTxFifo { ep: 1, .. }));
        assert!(fifo_action.is_some(), "should return WriteTxFifo action for EP1");

        // Verify data was written to EP1 TX FIFO
        let ep1_fifo_writes: Vec<_> = bank.writes.iter()
            .filter(|(off, _)| *off == ep_fifo(1))
            .collect();
        assert!(!ep1_fifo_writes.is_empty(), "data should be written to EP1 TX FIFO");
    }
    ...
    fn submit_control_stall() {
        ...
        assert!(
            actions.iter().any(|a| matches!(a, Dwc2Action::Stall { ep: 0 })),
    ...
    fn invalid_endpoint_rejected() {
        ...
        let result = ctrl.submit_request(
            GadgetRequest::BulkIn { ep: 5, data: alloc::vec![0u8] },
            &mut bank,
        );
