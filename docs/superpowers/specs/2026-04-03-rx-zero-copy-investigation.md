...
1. Allocates a `Vec<u8>` of `len` bytes
2. Copies from the DMA buffer via `ptr::copy_nonoverlapping`
3. Returns the Vec

The caller then frees the descriptor and reposts a receive buffer. The Vec is passed to one of three consumers:

- **smoltcp IP/ARP:** `netstack.ingest(frame: Vec<u8>)` — takes ownership, pushes into `VecDeque<Vec<u8>>` for smoltcp to read during `poll()`.
- **Harmony raw frames (Ring 1):** Slices into `&frame[6..12]` (MAC) and `&frame[ETH_HEADER_LEN..]` (payload), then `payload.to_vec()` into a `harmony_packets` Vec for later dispatch to the runtime.
- **Ring 3 `poll_network`:** Same `ingest(frame)` pattern as above.
...
Copy from DMA into a caller-provided slice instead of a new Vec. Avoids the heap allocation.

**Problem:** All consumers need to own the data beyond the call:
- `netstack.ingest()` takes `Vec<u8>` by value — would still need a Vec allocation
- Harmony path does `payload.to_vec()` — would still need a Vec allocation
- Net effect: replace one copy (DMA → heap) with two copies (DMA → stack → heap). **Strictly worse.**
...
The per-frame `Vec<u8>` allocation is the cheapest correct approach. The data must be copied out of the DMA buffer (descriptor reuse is non-negotiable), and all consumers need ownership. The allocation cost is one `alloc` + one `copy_nonoverlapping` of ~60-1500 bytes per frame.

**Revisit when:** Profiling (harmony-os-87o benchmarks) shows allocator pressure or RX processing latency as a bottleneck. Until then, this is optimal.
