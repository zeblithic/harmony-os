...
**Date:** 2026-03-19
**Status:** Implemented — stack switch + PQ keygen working on both architectures
**Bead:** harmony-os-wwa
...
**Root cause found:** PQ keygen on `x86_64-unknown-none` (without SSE2) needs >2MB of stack because the compiler cannot use XMM registers and spills aggressively during NTT polynomial operations. The fix is a 2MB stack — sufficient from the 4MB heap. This was initially misdiagnosed as a codegen issue (harmony-os-0vp) but is purely stack consumption from scalar (non-vectorized) code generation.

## Solution

Switch the kernel to a heap-allocated 2MB stack immediately after heap initialization. All subsequent boot code — PIT timer, entropy, identity generation (Ed25519), VirtIO, netstack, event loop — runs on the new stack.
...
kernel_main(boot_info):                          ← bootloader stack (~16KB)
  ...
  3. heap init (4MB)
  4. PIT timer init
  5. allocate 2MB stack from heap (forget the Vec)
  6. pack BootState into Box on heap
  7. asm trampoline: switch RSP, call kernel_continue
     ── stack boundary ──
kernel_continue(state: *mut BootState):          ← heap stack (2MB)
  8. unpack BootState
  9. RDRAND / entropy
  10. identity generation (Ed25519)
  11. generate PQ identity (ML-DSA-65 + ML-KEM-768)
  12. VirtIO, netstack
...
- `rsi` — new stack top (2MB allocation + size, 16-byte aligned)
...
heap base          ┌─────────────────────┐
                   ...
                   │  ┌─── RSP after ──┐ │
                   │  │    switch      │ │
heap base + 2M   └──┴───────────────┴──┘  ← stack_top (16-byte aligned)
```

- **Size:** 2MB. On `x86_64-unknown-none` without SSE2, PQ keygen needs >512KB of stack due to aggressive register spilling (no XMM registers available). 2MB provides headroom for future PQ operations (the four-layer key hierarchy, Nakaiah's integrity chains). Uses 50% of the 4MB heap.
- **Alignment:** `(heap_alloc_ptr + 2MB) & !0xF` — required by x86_64 SysV ABI. `Vec<u8>` has 1-byte alignment, so the base may be at any address. The `& !0xF` mask rounds down by at most 15 bytes, which is always within the 2MB allocation.
...
| File | Change |
|------|--------|
| `crates/harmony-boot/src/main.rs` | Split `kernel_main` into early-boot (steps 1-7) and `kernel_continue` (steps 8+). Add `BootState` struct. Add `switch_to_stack` asm. Move PIT init before the switch. PQ keygen runs on the 2MB heap stack. |
...
- Panic handler — uses `serial_init()` + port I/O. On the heap stack (post-switch), the panic handler has 2MB of stack space. On the bootloader stack (pre-switch), it has ~16KB — same as today, not a regression.
...
| `cargo xtask qemu-test --target x86_64` | Kernel boots on heap stack, PQ keygen runs, all 4 milestones pass |
